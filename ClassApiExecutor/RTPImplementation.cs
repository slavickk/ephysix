...
namespace CamundaInterface
{
        ...
        public RTPXmlTransport(string addr = @"http://10.74.28.40:25404", string password = "qwerty")
            ...
            public async Task<RTPDummySystem2> send(RTPDummySystem2 dummy2, string currentTopic = "")
            {
            /*    if (!string.IsNullOrEmpty(currentTopic) && !string.IsNullOrEmpty(NextChallenge) && !string.IsNullOrEmpty(session))
                {
                    var pwd = RTPDummySystem2.GetChallengePassword(NextChallenge, password);

                    dummy2.setPath($"DummySystem2/{currentTopic}Rq/Rq/@Password", pwd);
                    dummy2.setPath($"DummySystem2/{currentTopic}Rq/Rq/@Session", session);

                }*/
                string request1 = dummy2.outerXml;
                ...
                {
                    var ret = new RTPDummySystem2(await ans.Content.ReadAsStringAsync());
                    ...
                    {
                        session = ret.getPath("DummySystem2/InitSessionRp/Rp/Id");
                    }
        ...
        public async  Task beginSessionAsync()
        {
          /*  var currentKey = "InitSession";
            RTPDummySystem2 dummy2 = new RTPDummySystem2();
            dummy2.setPath("DummySystem2/InitSessionRq/Rq/NeedDicts", "0");
            dummy2.setPath("DummySystem2/InitSessionRq/Rq/AllVendors", "0");
            dummy2.setPath("DummySystem2/InitSessionRq/Rq/AvoidSession", "0");
            var ans = await send(dummy2, currentKey);
            //                var pwd =XmlDummySystem2.GetChallengePassword(tr.NextChallenge);
            RTPDummySystem2 dummySystem2Logon = new RTPDummySystem2();
            var ans11 = await send(dummySystem2Logon, "Logon");*/
        }
...
/*            string content;
            ...
            //            return new XmlDummySystem2(content);
            RTPDummySystem2 retValue = null;
            foreach (var com in commands)
            {
                RTPDummySystem2 rtpCommand = new RTPDummySystem2(com.CommandItem.environment);

                var currentKey = com.CommandItem;
                /*    dummySystem2Rate.setPath($"DummySystem2/{currentKey}Rq/Rq/@Password", pwd);
                    dummySystem2Rate.setPath($"DummySystem2/{currentKey}Rq/Rq/@Session", ans.getPath("DummySystem2/InitSessionRp/Rp/Id"));*/
...
//                foreach (var par in com.Params)
                    ...
                    /*                    if(par.Value.GetType() == typeof(JsonDocument))
                                        {
                                            ...
                                            if(jsonDocument.RootElement.ValueKind == JsonValueKind.Array)
                                            {
                                                ...
                                                while (users.MoveNext())
                                                {
                                                    ...
                                                    rtpCommand.setPath($"DummySystem2/{currentKey}Rq/Rq/{par.Key}/Row", null);
                                                    while (props.MoveNext())
                                                    {
                                                        var prop = props.Current;
                                                        if(prop.Value.ValueKind != JsonValueKind.Null)
                                                        rtpCommand.setPath($"DummySystem2/{currentKey}Rq/Rq/{par.Key}/Row/{prop.Name}",prop.Value.ToString());
                                                    }
                                                }
                                            } 
                    //                        if(jsonDocument.RootElement. != null)
                    //                        dummySystem2Command.setPath($"DummySystem2/{currentKey}Rq/Rq/{par.Key}", par.Value.ToString());
        ...
        {
            /*RTPDummySystem2 dummySystem2Logoff = new RTPDummySystem2();
            var ans11 = await send(dummySystem2Logoff, "Logoff");*/
        }
...
//            string pattern = "Data/RPT/*.xml";
            foreach (var filePath in Directory.GetFiles("Data/RTP", "*.xml"))
            {
                ...
                xmlDoc.LoadXml(RTPDummySystem2.TransformXml(xmlContent));
...
//            throw new NotImplementedException();
        ...
        public class RTPDummySystem2 : _ApiFilter
    {
...
//                    RTPXmlTransport.FormName(node, name);
        ...
        static byte[] hashPWD(string password, string text)
            ...
            public RTPDummySystem2()
            {
                string xmlContent = @"<env:Envelope xmlns:env='http://schemas.xmlsoap.org/soap/envelope/' xmlns:m1='http://schemas.compassplus.com/two/1.0/dummy2.xsd' xmlns:m0='http://schemas.compassplus.com/two/1.0/dummy_systemm2_types.xsd'>
  <env:Body>
    <m1:InitSessionRq>
      <m1:Request Ver='9.1' Product='DummySystem2' RetAddress='192.166.22.33' Clerk='DummySystem2XML'>
      </m1:Request>
    </m1:InitSessionRq>
  </env:Body>
</env:Envelope>";
            ...
            public RTPDummySystem2(string xmlContent)
            {
                Init(xmlContent);
            }
...
//            xmlContent = xmlContent.Replace("</Tran", "</a1:Tran");
            ...
            /*                namespaces.Add("env", "http://schemas.xmlsoap.org/soap/envelope/");
                            ...
                            namespaces.Add("m1", "http://schemas.compassplus.com/two/1.0/dummy2.xsd");*/

            var el = xmlDoc.DocumentElement;
            AddNameSpaces(el);

            /*            namespaces.Add("m0", "http://schemas.compassplus.com/two/1.0/dummy_systemm2_types.xsd");
                        namespaces.Add("m1", "http://schemas.compassplus.com/two/1.0/dummy2.xsd");
                        namespaces.Add("m", "http://schemas.compassplus.com/two/1.0/dummy2.wsdl");*/
...
//                if (!string.IsNullOrEmpty(item.Key))
  ...
  /*              else
      ...
      /*  public static string XmlPath(string path)
            {
                var lex = path.Replace("DummySystem2/", "").Replace("/Rq/", "/m1:Request/").Replace("/Rp/", "/Response/").Split("/").ToList();
                lex[0] = "m1:" + lex[0];
