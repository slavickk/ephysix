                ...
                foreach (var ruleInput in rule.Inputs)
                {
                    ...
                    if (allowedValues != null && allowedValues.Length > 0)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(ruleInput.Input.Expression))
                        {
                            ...
                            var valueObj = context.EvalExpression<object>(ruleInput.Input.Expression, executionId, this.Name);
                    ...
                    var result = context.EvalExpression<bool>(ruleInput.Expression, executionId, this.Name);
                    if (Logger.IsTraceEnabled)
                        Logger.TraceCorr(executionId,
        ...
        private DmnDecisionTableRuleExecutionResults EvaluateOutputsForPositiveRules(DmnExecutionContext context, string executionId, IEnumerable<DmnDecisionTableRule> positiveRules)
        {
            ...
            foreach (var positiveRule in positiveRules)
            {
                ...
                foreach (var ruleOutput in positiveRule.Outputs)
                {
                    ...
                    var result = context.EvalExpression(ruleOutput.Expression, ruleOutput.Output.Variable.Type ?? typeof(object), executionId, this.Name);
