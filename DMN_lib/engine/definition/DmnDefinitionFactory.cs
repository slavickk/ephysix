...
namespace net.adamec.lib.common.dmn.engine.engine.definition
{
    ...
    public class DmnDefinitionFactory
    {
        ...
        string checkExternalVar(string expression,string type)
        {
            switch (expression)
            {
                case "season":
        ...
        public static string getRootVariable(string input)
        {
            ...
            List<IDmnDecision> requiredDecisions)
        {
            ...
            foreach (var sourceInput in sourceTable.Inputs)
            {
                ...
                {
                    /* Modified by Gasnikov*/
                    expression = checkExternalVar(sourceInput.InputExpression.Text, sourceInput.InputExpression.TypeRef);
                    if (string.IsNullOrWhiteSpace(expression))
                    {
                        if (sourceInput.Label == null && sourceInput.InputExpression?.Text != null)
                            expression = sourceInput.InputExpression.Text;
                        else
                            expression = DmnVariableDefinition.NormalizeVariableName(sourceInput.Label);// sourceInput.InputExpression.Text;
                    }
                    /* Modified by Gasnikov*/
                }
        ...
        private static Type ParseTypeName(string typeName)
        {
            ...
            switch (typeName)
            {
                ...
                case "number": return typeof(long);
