...
namespace net.adamec.lib.common.dmn.engine.engine.execution.context
{
    ...
    public class DmnExecutionContext
    {
        ...
        bool prepared = false;
        List<IDmnDecision> calcDecisions;
        public void ExecDecisions()
        {
            if (!prepared)
            {
                List<IDmnDecision> requiredDecisions = new List<IDmnDecision>();
                foreach (var dec in this.Decisions)
                    requiredDecisions.AddRange(dec.Value.RequiredDecisions);
                calcDecisions = new List<IDmnDecision>();
                calcDecisions = this.Decisions.Where(dec => !requiredDecisions.Contains(dec.Value)).Select(ii => ii.Value).ToList();
                prepared = true;
            }
            foreach (var dec in calcDecisions)
            {
                DateTime timeEnd = DateTime.Now;
                this.ExecuteDecision(dec);
            }
        }
        ...
        public virtual object EvalExpression(string expression, Type outputType, string executionId,string ownerName)
        {
            ...
            {
                CheckInterpreterParameters(parameters);
            }
                ...
                !GetParsedExpressionsFromCache(cacheKey, out var parsedExpression))
            {
                ...
                {
                    throw new DmnExecutorException(ownerName, expression, $"Exception while parsing the expression {expression}", exception);
                   /* Logger.Fatal<DmnExecutorException>($"Exception while parsing the expression {expression}",
                         exception);*/
                }
            ...
            period += (DateTime.Now - time1).TotalMilliseconds;
  ...
  //              parameter.Value
        ...
        protected  void CheckInterpreterParameters(List<Parameter> parameters)
        {
            //Prepare variables (as interpreter parameters)       
            // ReSharper disable once LoopCanBeConvertedToQuery
            for (int i = 0; i < parameters.Count; i++)
            {
                var varValue = Variables[parameters[i].Name].Value;
                if (varValue != null && varValue != parameters[i].Value)
                {
        ...
        public TOutputType EvalExpression<TOutputType>(string expression, string executionId,string ownerName)
        {
            if (string.IsNullOrWhiteSpace(expression))
                throw Logger.Fatal<ArgumentNullException>($"EvalExpression: - {nameof(expression)} is null or empty");

            var result = EvalExpression(expression, typeof(TOutputType), executionId,ownerName);
