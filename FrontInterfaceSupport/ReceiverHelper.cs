...
//                                            return list.Where(ii=>ii.Name == filterName && (string.IsNullOrEmpty(filterValue) || filterValue.Equals(ii.Value)));
        ...
        public static async Task<MXGraphDoc.Box.Body> getItem(string boxIdPrefix="aa",string IID= "C:\\D\\OUT_TIC\\UniElLib.ContextItemStep_0Receiver_po2vgdbb.2dk")
        {
            using(StreamReader sr = new StreamReader(IID))
            {
                var line = sr.ReadToEnd();
