...
using DummyProtocol3;

namespace ParserLibrary.DummyProtocol2
{
    /// <summary>
    /// Receiver for DummyProtocol2 files
    /// </summary>
    public class DummyProtocol2Receiver : Receiver, IDisposable
    {
        ...
        static DummyProtocol2Receiver()
        {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
        }

        public DummyProtocol2Receiver()
        {
            logContext = LogContext.PushProperty("receiver", "DummyProtocol2");
        }


        public string tracking_dir;
        /// <summary>
        /// Work dir for DummyProtocol2. 
        /// </summary>
        /// <remarks>
        ...
        /// <item>
        /// <term>upload</term>
        /// <description> DummyProtocol2 watching this directory and load new files to system</description>
        /// </item>
        /// <item>
        /// <term>processed</term>
        ...
        public override async Task startInternal()
        {
            ...
            foreach(var file in Directory.GetFiles(upload_dir))
            {
                ...
                await ParseDummyProtocol2file(Path.GetFileName(file));
        ...
        private async void WatcherOnCreated(object sender, FileSystemEventArgs e)
        {
            using (LogContext.PushProperty("FileName", e.Name))
            {
                ...
                if (ValidateFile(e.FullPath))
                {
                    ...
                    await ParseDummyProtocol2file(e.Name);
        ...
        private async Task ParseDummyProtocol2file(string fileName)
        {
            ...
            try
            {
                using (FileStream fileStream = File.OpenRead(upload_file))
                {
                    ...
                    do
                    {
                        ...
                        {
                            fileStream.Close();
                            Log.Error("Not DummyProtocol2 message");
                    ...
                    DummyProtocol3Message header;
                    Encoding encoding;

                    try
                    {
                        header = new DummyProtocol3Message(Encoding.UTF8.GetString(buffer.ToArray()));
                        encoding = (header["Encoding"] as DummyProtocol3Scalar)!.Value.ToLower() switch
                        {
                    ...
                    do
                    {
                        ...
                        if (b == (int) Symbols.MS)
                        {
                            ...
                            {
                                recieved_rows++;
                                var send = new DummyProtocol3Message();
                                DummyProtocol3Message message = new DummyProtocol3Message(encoding.GetString(buffer.ToArray()));
                                send["header"] = header;
