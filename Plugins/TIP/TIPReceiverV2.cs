...
public class TIPReceiverV2 : IReceiver, IDisposable
{
    ...
    /// <summary>
    /// Starts the receiver.
    /// Corresponds to the startInternal() method of the original receiver.
    /// </summary>
                ...
                do
                {
                    ...
                    if (b == (int) Symbols.MS)
                    {
                        try
                        {
                            ...
                            await host.signal(JsonSerializer.Serialize(send), fileName);
                            send_rows++;
                            buffer.Clear();
                            continue;
                        }
    ...
    {
        throw new NotImplementedException("TIPReceiver is not supposed to send responses");
    }
