...
namespace TestJsonRazbor
{
    ...
    public partial class FormStreamSenderSetup : Form, SenderDataExchanger
    {
        ...
        {
            return new StreamSender.Stream() { Name = textBoxName.Text, Description = textBoxDescription.Text, fields = fields.Select(ii => new StreamSender.Stream.Field() { Name = ii.Name, Detail = ii.Detail, Type = ii.Type, linkedColumn = ii.refColumn?.col_id }).ToList() };
        }
...
where nc.name like '%" + textBox1.Text + "%' and nc.isdeleted=false and nc.srcid=2", conn))
        ...
        {
            fields.Add(new FieldItem() { Detail = textBoxFieldDescription.Text, Hash = checkBoxHash.Checked, Name = textBoxFieldName.Text, Type = comboBoxFieldType.SelectedItem?.ToString(), refColumn = linkedColumn });
            RefreshListFields();
        }
        ...
        private void RefreshListFields()
        {
            ...
            listView1.Items.AddRange(fields.Select(ii => new ListViewItem(new string[] { ii.Name, ii.Type, ii.Detail, ii.Hash.ToString(), ii.refColumn?.ToString() })).ToArray());
        }
        ...
        private void listView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(listView1.SelectedIndices.Count > 0)
            {
                ...
                checkBoxHash.Checked = fields[index].Hash;
        ...
        private void setContent(StreamSender.Stream st)
        {
            ...
            fields.Clear();
            fields.AddRange(st.fields.Select(ii => new FieldItem() { Name = ii.Name, Detail = ii.Detail, Type = ii.Type, Hash = ii.Hash }));
            RefreshListFields();
        }
