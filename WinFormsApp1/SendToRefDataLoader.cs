...
namespace WinFormsApp1
{
    public class SendToRefDataLoader
    {
       ...
       string.IsNullOrEmpty(text) ? string.Empty : BitConverter.ToString(new System.Security.Cryptography.SHA256Managed().ComputeHash(System.Text.Encoding.UTF8.GetBytes(text))).Replace("-", string.Empty);
        ...
        public static async Task putRequestToRefDataLoader(HttpClient client,string processId="asdfa" ,string connectionStringBase = "User ID=postgres;Password=test;Host=localhost;Port=5432;", string connectionStringAdmin = "User ID=fp;Password=rav1234;Host=192.168.75.220;Port=5432;Database=fpdb;",
            string dictName = "People", string FID = "TEST",string command= "SELECT id  ID1,firstname,middlename,lastname,sex FROM public.aa_person", int maxRecord=500,string baseAddr= "http://192.168.75.212:21525")
        {
